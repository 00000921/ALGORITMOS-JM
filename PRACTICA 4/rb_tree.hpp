    ...
    while (node->parent->color == RED)
    {
        if (node->parent == node->parent->parent->left)
        {
            ...
            if (uncle->color == RED)
            {
                ...
                node = node->parent->parent; // WARNING
            }
            else
            {
                if (node == node->parent->right)
                {
                    node = node->parent; // WARNING
                    LeftRotation(node);
                }
    ...
    while (node != this->root && node->color == RED)
    {
        if (node == node->parent->left)
        {
            ...
            else
            {
                ...
                // Set color of sibling to node.parent.color
                sibling->color = node->parent->color;
                node->parent->color = BLACK;
                sibling->right->color = BLACK;
                LeftRotation(node->parent);
                node = this->root; // WARNING
            }
        }
        else
        {
...
// Funcion para mostrar las otras funciones
...
// Funcion para mostrar informacion general
